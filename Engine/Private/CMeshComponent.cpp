...
bool CMeshComponent::Intersects_Ray(_vector origin, _vector rayDir, _float& Dist)
{
	...
	for (_uint i = 0; i < m_pIndices.size(); i += 3)
	{
		...
		_vector p1 = XMLoadFloat3(&m_pPositions[m_pIndices[(UINT64)i + 1]]);
		_vector p2 = XMLoadFloat3(&m_pPositions[m_pIndices[(UINT64)i + 2]]);
...
HRESULT CMeshComponent::Bind_Bones(CShader* pShader, const _char* pConstatName, const vector<CBone*>& Bones)
{
	...
	/*한번에 떤진다.*/
	m_pGameInstance->CopyData_Buffer("BoneBuffer", &m_BoneMatrices, sizeof(m_BoneMatrices));
	return S_OK;
}
